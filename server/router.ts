...
// --- 高階 AI 藥物分析端點 ---
router.post('/analyze-medication', (req, res) => {
  try {
    ...
    res.status(500).json({ error: '分析失敗，請稍後重試' });
  }
});

// --- 醫療地點查詢端點 ---
router.get('/nearby-medical-locations', (req, res) => {
  try {
    ...
    res.status(500).json({ error: '查詢失敗，請稍後重試' });
  }
});
