...
import { Lock, Mail, CheckCircle2, AlertCircle, LayoutDashboard, Pill, Calendar, User, LogOut, ChevronLeft, Plus, Bell, Settings, Trash2, Edit2, Clock, ShieldCheck, UserPlus, LogIn, KeyRound, Eye, EyeOff } from "lucide-react";
...
export const PasswordSettings: React.FC = () => {
        ...
        return (
          <div className="p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-8">
              ...
              <button className="bg-blue-600 text-white px-6 py-3 rounded-2xl flex items-center gap-2 hover:bg-blue-700 shadow-lg shadow-blue-100 transition active:scale-95 font-bold">
                <Plus size={20} /> 新增藥品
              </button>
            </div>
                ...
                <div key={med.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-center hover:shadow-md transition">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="text-gray-500 font-medium">{med.dosage} • {med.frequency} • {med.time}</div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button className="p-3 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition"><Edit2 size={20} /></button>
                    <button onClick={() => deleteMed(med.id)} className="p-3 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition"><Trash2 size={20} /></button>
                  </div>
                </div>
              ))}
              {meds.length === 0 && (
            ...
            <div className="space-y-4">
              {reminders.map(r => (
                <div key={r.id} onClick={() => toggleReminder(r.id)} className={`p-6 rounded-3xl border transition-all cursor-pointer flex justify-between items-center ${r.taken ? "bg-green-50 border-green-100 opacity-75" : "bg-white border-gray-100 shadow-sm hover:shadow-md"}`}>
                  <div className="flex items-center gap-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${r.taken ? "bg-green-600 text-white" : "bg-gray-100 text-gray-400"}`}>
                      <Clock size={28} />
                    </div>
                    <div>
                      <div className={`text-xl font-bold ${r.taken ? "text-green-800 line-through" : "text-gray-900"}`}>{r.medName}</div>
                      <div className="text-gray-500 font-medium">提醒時間：{r.time}</div>
                    </div>
                  </div>
                  {r.taken ? (
                    <div className="flex items-center gap-2 text-green-600 font-bold bg-white px-4 py-2 rounded-full shadow-sm">
                      <CheckCircle2 size={20} /> 已服用
                    </div>
                  ) : (
                    <div className="text-gray-400 font-bold border-2 border-gray-100 px-4 py-2 rounded-full">點擊標記已服用</div>
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="px-8 pb-8">
                <div className="relative -top-12 flex items-end gap-6">
                  <div className="w-32 h-32 bg-white p-2 rounded-3xl shadow-lg">
                    <div className="w-full h-full bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center text-4xl font-black">用</div>
                  </div>
                  <div className="pb-4">
        ...
        return (
          <div className="p-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div onClick={() => setCurrentSubPage("meds")} className="p-8 bg-blue-50 rounded-[2.5rem] border border-blue-100 hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group">
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-3">藥品管理</h3>
                <p className="text-gray-600 leading-relaxed">新增、修改或刪除您的常用藥品清單。</p>
              ...
              <div onClick={() => setCurrentSubPage("reminders")} className="p-8 bg-green-50 rounded-[2.5rem] border border-green-100 hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group">
                <div className="w-16 h-16 bg-green-600 text-white rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-green-200 group-hover:rotate-12 transition-transform">
                  <Calendar size={32} />
                </div>
                <h3 className="text-2xl font-black text-gray-900 mb-3">用藥提醒</h3>
                <p className="text-gray-600 leading-relaxed">設定每日用藥時間，系統將準時提醒您。</p>
              ...
              <div onClick={() => setCurrentSubPage("profile")} className="p-8 bg-purple-50 rounded-[2.5rem] border border-purple-100 hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group">
                <div className="w-16 h-16 bg-purple-600 text-white rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-purple-200 group-hover:rotate-12 transition-transform">
                  <User size={32} />
                </div>
                <h3 className="text-2xl font-black text-gray-900 mb-3">個人檔案</h3>
                <p className="text-gray-600 leading-relaxed">管理您的健康資訊與緊急聯絡人。</p>
            ...
            <div className="mt-16 p-10 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm text-center">
          ...
          <div className="p-10">
            ...
            {authMode === "reset" ? (
              <form onSubmit={handleReset} className="space-y-6">
                ...
                {testCode && (
                  <div className="space-y-6 animate-in fade-in duration-300">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={authData.code} onChange={(e) => setAuthData({ ...authData, code: e.target.value })} className="w-full h-14 bg-gray-50 border-none rounded-2xl px-4 focus:ring-2 focus:ring-blue-600 font-black text-center text-2xl tracking-[1em]" placeholder="000000" maxLength={6} required />
                    </div>
                ...
                <button type="button" onClick={() => { setAuthMode("login"); setTestCode(null); }} className="w-full text-gray-400 font-bold hover:text-blue-600 transition">返回登入</button>
              </form>
            ) : (
              <form onSubmit={handleAuth} className="space-y-6">
                ...
                {authMode === "login" && (
                  <div className="text-right">
                    <button type="button" onClick={() => setAuthMode("reset")} className="text-sm font-bold text-blue-600 hover:underline">忘記密碼？</button>
                  </div>
                )}
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="bg-white rounded-[3rem] shadow-2xl shadow-blue-50 overflow-hidden border border-gray-100">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-10 text-white">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-3xl font-black tracking-tight">控制中心</h2>
            </div>
            <button onClick={() => setIsLoggedIn(false)} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-5 py-2.5 rounded-2xl transition font-bold backdrop-blur-md border border-white/10">
              <LogOut size={20} /> 登出
            </button>
          </div>
          <p className="text-blue-100 text-lg font-medium">歡迎回來，今天也要準時服藥喔！</p>
        </div>
        {renderContent()}
      </div>
    </div>
